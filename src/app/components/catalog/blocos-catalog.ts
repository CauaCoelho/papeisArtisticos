...
export class BlocosCatalog implements OnInit {
  ...
  ngOnInit(): void {
    this.blocoService.findAll(0, 100).subscribe({
      next: (data: any) => {
      ...
      error: (err) => {
        this.errorMessage.set(`Erro ao carregar blocos: ${err.status}`);
        this.loading.set(false);
      }
    });
  }
